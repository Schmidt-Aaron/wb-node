...
function autocomplete(input, latInput, lngInput) {
    if (!input) return; //skip from running if there is no input on the page
    ...
    //if someone hits enteron address field, dont sumbit the form
...
function typeAhead(search) {
  ...
  searchInput.on('keyup', function (e) {
    // if they arent pressing up, down or enter...
...
function loadPlaces(map) {
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    // then zoom the map to fill all the markers
